---
name: memgraph-init
description: One-shot configurator that wires memgraph into a CLAUDE.md (global or project-local) so the agent uses the persistent memory consistently in every session. Asks the user 4 short questions (scope, caching, retrieve strategy, save strategy), then writes a `<!-- memgraph:start -->` … `<!-- memgraph:end -->` block into the chosen CLAUDE.md. Triggered by `/memgraph-init`, "configura memgraph", "set up memgraph for this project", "enable memgraph globally". Idempotent — re-running it replaces the previous block in place.
---

# memgraph-init — Configure memgraph behavior in CLAUDE.md

This skill writes a short, opinionated instruction block into a CLAUDE.md so future sessions automatically use the memgraph skills (`recall`, `memoryze`, `learn`, `memory-audit`) according to the user's preferences. The block is fenced with HTML markers so re-running the skill cleanly replaces it.
...
```
question: "Where should the memgraph configuration live?"
header:   "Scope"
options:
...
header:   "Caching"
options:
  - { label: "Enable (recommended)", description: "Use `memgraph query` first (fast, exact-match cache) before escalating to retrieve. Cuts latency on repeat questions." }
...
```markdown
<!-- memgraph:start -->
## memgraph — persistent memory (auto-configured by /memgraph-init)

You have a persistent memory graph (`memgraph`) shared across sessions. Use it actively — it is your first stop on non-trivial work and the place where hard-won knowledge accumulates.

**Retrieve:** {{retrieve_clause}}
**Cache:** {{cache_clause}}
**Save:** {{save_clause}}

Companion skills: `/recall <q>` (search), `/memoryze <hints>` (save from conversation), `/learn <prompt>` (bulk-ingest external sources), `/memory-audit` (health check). Prefer these slash commands over raw `memgraph` CLI calls.
...
| Choice | `cache_clause` |
| ------ | -------------- |
| Enable | "Use `memgraph query` first for fast exact-match lookups; escalate to `retrieve` / `explore` only on miss." |
...
- If it contains `<!-- memgraph:start -->` … `<!-- memgraph:end -->`, replace **exactly that span** (markers included) with the new block. Do not touch anything else in the file.
...
1. `Wrote memgraph config to <absolute path>.` (one line)
