#ifndef MEMGRAPH_CLI_PROFILE_H
#define MEMGRAPH_CLI_PROFILE_H

#include <stddef.h>

/* Profiles tenant-isolate the memory graph. Each profile is a directory
 * under MEMGRAPH_HOME (`$HOME/.lmemorygraph` POSIX, same on Windows)
 * holding its own SQLite DB and getting its own daemon listening on its
 * own AF_UNIX socket.
 *
 * The active profile is whatever $MEMGRAPH_PROFILE says, or "default" if
 * the env var is unset. `memgraph profile set <name>` does NOT mutate
 ...
 * honors MEMGRAPH_SOCKET / MEMGRAPH_DB_PATH env overrides set by the CLI.
 */

#define MG_PROFILE_DEFAULT "default"

/* Resolve MEMGRAPH_HOME. Creates the directory if missing. */
 ...
 * `db`    = <home>/profiles/<name>/memgraph.db
...
/* Dispatcher for `memgraph profile <subcommand> ...`.
 * argv[0]=memgraph, argv[1]=profile, argv[2]=subcommand, ... */
